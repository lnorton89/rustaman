...
//
// Dependencies: anyhow, std::fs (walks src/, tests/ and examples/ at test time)
// ============================================================================
...
//! nothing else can imply: a reader opening a module cold gets its path
//! in the crate, what it is for, and what it leans on, before reading a
//! line of code. A convention only holds if something checks it, so this
//! walks `src/`, `tests/` and `examples/` and fails the build for any
//! file whose header is missing, malformed, or naming the wrong module.
//!
//! The check is structural, not editorial. It verifies that the fields
//! are present and filled in, and that `Module:` matches the path the
//! file actually sits at — which is what catches a header copied from a
...
//! `//!` block below it is the documentation, and that is where the
//! reasoning belongs.
...
mod tests {
    ...
    /// The rule above and below every header. Both edges are checked, so
    /// a header cannot silently run on into the code below it.
    const RULE: &str =
        "// ============================================================================";

    /// Width of the label column, so `Description:` and its continuation
    /// lines share one left edge. Kept in agreement with the headers by
    /// the continuation parsing in [`field`], which accepts only this
    /// indent.
    const LABEL_WIDTH: usize = 14;

    /// Fields every header must carry.
    ...
    fn expected_module(relative: &str) -> Result<String> {
        let (area, rest) = relative
            .split_once('/')
            .ok_or_else(|| anyhow!("{relative} is not under a checked directory"))?;
        let stem = rest
            .strip_suffix(".rs")
            .ok_or_else(|| anyhow!("{relative} is not a Rust source file"))?;

        match area {
            "tests" => return Ok(format!("{stem} (integration test)")),
            "examples" => return Ok(format!("{stem} (example)")),
            _ => {}
        }
        if let Some(binary) = stem.strip_prefix("bin/") {
            return Ok(format!("rustaman-{binary} (binary crate root)"));
        }
        match stem {
            "lib" => return Ok("rustaman (library crate root)".to_string()),
            "main" => return Ok("rustaman (binary crate root)".to_string()),
            _ => {}
        }
    ...
    fn problems(relative: &str, text: &str) -> Vec<String> {
        ...
        let Some(body) = lines.get(1..=close) else {
            found.push(format!("{relative}: header body could not be read"));
            return found;
        };
    ...
    fn a_module_path_is_derived_from_where_the_file_sits() -> Result<()> {
        let cases = [
            ("src/lib.rs", "rustaman (library crate root)"),
            ("src/main.rs", "rustaman (binary crate root)"),
            ("src/color.rs", "color"),
            ("src/model/mod.rs", "model"),
            ("src/model/sort.rs", "model::sort"),
            ("src/win/nt/process.rs", "win::nt::process"),
            ("src/gui/ui/theme.rs", "gui::ui::theme"),
            ("tests/smoke.rs", "smoke (integration test)"),
            ("examples/brand_assets.rs", "brand_assets (example)"),
        ];
    ...
    #[test]
    fn a_file_that_does_not_open_with_a_header_is_rejected() {
        let found = problems("src/color.rs", "//! Just docs, no banner.\n");
        assert!(
            found
                .iter()
                .any(|problem| problem.contains("does not open with")),
            "got {found:?}"
        );

        let unclosed = format!("{RULE}\n// Module:       color\n");
        let found = problems("src/color.rs", &unclosed);
        assert!(
            found.iter().any(|problem| problem.contains("never closed")),
            "a header that runs on into the code should be reported, got \
             {found:?}"
        );
    }
    ...
    #[test]
    fn an_overlong_header_line_is_rejected() {
        // A header that runs past rustfmt's width wraps in an editor and
        // stops lining up, which is the whole point of the column.
        let long = "x".repeat(MAX_WIDTH);
        let text = format!(
            "{RULE}\n\
             // Module:       color\n\
             // Description:  {long}\n\
        ...
        let found = problems("src/color.rs", &text);
        assert!(
            found.iter().any(|problem| problem.contains("columns")),
            "got {found:?}"
        );
    }
}
